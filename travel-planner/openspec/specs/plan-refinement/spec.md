# plan-refinement Specification

## Purpose
让用户在首次生成旅行计划后,通过携带 `session_id` 的多轮追问,持续在上一版计划基础上追加/修改约束(预算、天数、目的地、日期、偏好),系统按需重检索、重算费用并重排行程;每天细化为带时段/类别的结构化活动,状态持久化到服务端 SQLite,演示模式下同样可离线细化。
